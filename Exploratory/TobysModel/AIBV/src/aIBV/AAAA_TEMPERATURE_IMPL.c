...
void AAAA_TEMPERATURE_IMPL()
{
	while(TRUE)
	{
		...
		if ((AIBV_temperature_needs_checking_AAAA_TEMPERATURE_IMPL == TRUE) && ((MIN_TEMP <= AIBV_buffered_temperature_AAAA_TEMPERATURE_IMPL) && (AIBV_buffered_temperature_AAAA_TEMPERATURE_IMPL <= MAX_TEMP)) && ((MIN_TEMP <= buffered_temperature) && (buffered_temperature <= MAX_TEMP)))
		{
