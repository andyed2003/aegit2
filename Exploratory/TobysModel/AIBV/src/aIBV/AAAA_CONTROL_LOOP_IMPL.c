...
void AAAA_CONTROL_LOOP_IMPL()
{
	while(TRUE)
	{
		...
		if ((AIBV_state_AAAA_CONTROL_LOOP_IMPL != DISARMED) && (AIBV_action_AAAA_CONTROL_LOOP_IMPL == CLOSE) && (AIBV_control_action_started_AAAA_CONTROL_LOOP_IMPL == FALSE) && (0 <= AIBV_amount_AAAA_CONTROL_LOOP_IMPL))
		{
			AIBV_control_action_started_AAAA_CONTROL_LOOP_IMPL = TRUE;
		}
		else
		...
		if ((AIBV_action_needs_to_measure_position_AAAA_CONTROL_LOOP_IMPL == TRUE) && (AIBV_action_AAAA_CONTROL_LOOP_IMPL == NOTHING) && (((AIBV_CLOSED <= position_measurement) && (position_measurement <= AIBV_OPEN)) || (position_measurement == AIBV_UNKNOWN)) && (AIBV_measuring_position_AAAA_CONTROL_LOOP_IMPL == TRUE))
		{
